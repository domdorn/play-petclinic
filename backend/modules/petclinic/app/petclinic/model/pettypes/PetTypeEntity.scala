package petclinic.model.pettypes
...
object PetTypeProtocol {
  ...
  private[pettypes] sealed trait Event
  ...
  private[pettypes] case class PetTypeCreatedEvent(id: String, name: String) extends Event
  ...
  private[pettypes] case class PetTypeDeletedEvent(id: String, name: String)
  ...
  private[pettypes] case class PetTypeNameChangedEvent(id: String, name: String) extends Event


  /* private[pettypes] */case class PetTypeVO(id: String, name: String)
}

class PetTypeEntity extends PersistentActor {
  override def persistenceId: String = "pettypes"

  var petTypes: Map[PetTypeVO, Int] = Map()
  var latestId = 1

  override def receiveRecover: Receive = {
    case PetTypeCreatedEvent(id, name) =>
      petTypes = petTypes + (PetTypeVO(id, name) -> 0)
      latestId = latestId + 1
    case PetTypeDeletedEvent(id, name) => petTypes = petTypes - PetTypeVO(id, name)
    case PetTypeNameChangedEvent(id, newName) => {
      petTypes.find(_._1.id == id) match {
        case Some(entry) => petTypes = (petTypes - entry._1) + (PetTypeVO(id, newName) -> entry._2)
