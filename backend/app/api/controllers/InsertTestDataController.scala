...
import api.owners.createOwner.CreateOwnerService
import api.owners.getAllOwners.{GetAllOwnersService, OwnerGetApiResponse}
import api.pets.addPet.AddPetService
...
class InsertTestDataController @Inject()(
  ...
  def insertPetTypes() = controllerComponents.actionBuilder.async { req =>
    Future.sequence(
      Seq(
        petTypeService.create(Cat),
        petTypeService.create(Dog),
        petTypeService.create(Lizard),
        petTypeService.create(Snake),
        petTypeService.create(Crocodile),
        petTypeService.create(Elephant),
        petTypeService.create(Hamster),
        petTypeService.create(Mouse)
      ).map(_.toScala)
  ...
  def insertPets() = controllerComponents.actionBuilder.async { req =>
    val x = getAllOwnersService.getOwners.toScala
      .zip(petTypeService.getAll.toScala)
