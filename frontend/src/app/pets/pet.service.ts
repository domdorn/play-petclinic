...
export class PetService {
  ...
  getPetById(pet_id: string, owner_id: string): Observable<Pet> {
    return this._http.get(this.entity_url + '/' + owner_id + "/" + pet_id)
      .map((response: Response) => <Pet> response.json())
      .catch(this.handleError);
  }
  ...
  updatePet(pet_id: string, owner_id: string, pet: Pet): Observable<Pet> {
    const body = JSON.stringify(pet);
    const headers = new Headers({'Content-Type': ' application/json;charset=UTF-8'});
    const options = new RequestOptions({headers: headers});
    return this._http.put((this.entity_url + '/' + owner_id + "/" + pet_id), body, options)
